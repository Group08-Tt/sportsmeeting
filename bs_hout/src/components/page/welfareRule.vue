    ...
    <div>
        <div class='crumbs'>
            <el-breadcrumb separator='/'>
                <el-breadcrumb-item>
                    <i class='el-icon-lx-cascades'></i> 法律法规
                </el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class='container'>
...
<script>
import welfareRuleApi from '@/api/welfareRuleApi';
...
export default {
    ...
    methods: {
        ...
        // 获取 easy-mock 的模拟数据
        getData() {
            welfareRuleApi.list(this.query).then(({ code, msg, data }) => {
                if (200 !== code) {
        ...
        handleDelete(index, row) {
            ...
            this.$confirm('确定要删除吗？', '提示', {
                ...
                welfareRuleApi.batchDelete({ ids: [row.id] }).then(({ code }) => {
                    if (200 !== code) {
        ...
        delAllSelection() {
            ...
            welfareRuleApi.batchDelete({ ids: arr }).then(({ code }) => {
                if (200 !== code) {
                    this.$message.error('删除失败!');
                    return;
                }
        ...
        saveEdit() {
            ...
            this.$refs['form'].validate((valid) => {
                ...
                if (this.form.id === 0 || this.form.id === undefined) {
                    welfareRuleApi.addType(this.form).then(({ code, msg, data }) => {
                        if (200 !== code) {
                    ...
                    welfareRuleApi.update(this.form).then(({ code, msg, data }) => {
                        if (200 !== code) {
