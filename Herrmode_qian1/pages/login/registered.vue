		...
		<view>
			...
			<view class="login-content">
				...
				<view class="common">
					...
					<input class="com-inp" type="text" v-model="dataForm.username" placeholder="姓名" />
				</view>

				<view class="common">
					<view class="iconCss iconfont icon-mima"></view>
					<input class="com-inp" type="text" v-model="dataForm.sex" placeholder="性别" />
				</view>

				<view class="common">
					<view class="iconCss iconfont icon-mima"></view>
					<input class="com-inp" type="number" v-model="dataForm.studentid" placeholder="学号" />
				</view>

				<view class="common">
					<view class="iconCss iconfont icon-mima"></view>
					<input class="com-inp" type="text" v-model="dataForm.college" placeholder="学院" />
				</view>

				<view class="common">
					<view class="iconCss iconfont icon-mima"></view>
					<input class="com-inp" type="number" v-model="dataForm.iphonenumber" placeholder="手机号" />
				</view>
	...
	export default {
		...
		methods: {
			...
			submit() {
				...
				if (!isokquery || !ischeckEmail || !this.isShow || dataForm.username == "" || dataForm.studentid == "" ||
					dataForm.college == "" || dataForm.iphonenumber == "" || dataForm.captcha == "") {
					if (!ischeckEmail) { //意思是邮箱错误
						...
						this.showMsg("请检查信息是否完整")
						return
					}
				}
				//下面验证信息 ：  两次密码验证、
				this.$http.request({
					url: this.$api.account_login_register_find.Email_register,
					data: {
						username: dataForm.username, //用户的名称
						gender: dataForm.sex, //用户的性别
						email: dataForm.mobile, //用户的邮箱
						password: dataForm.password, //用户的密码
						code: dataForm.captcha, //用户输入的验证码
						studentid: dataForm.studentid, //学生的id
						college: dataForm.college, //学院
						iphonenumber: dataForm.iphonenumber, //手机号
					},
					method: 'POST'
				}).then(res => {
					console.log(res)
					if (res.start === 1) { //说明注册成功了
			...
			getCode() {
				...
				this.$http.request({
					url: this.$api.account_login_register_find.Email_register_emverrigin,
					data: {
